const copy = {
  en: {
    ...
    q5:'Can I drive Claude from my phone?', a5:'Yes, with Claude Code’s built-in <a href="https://code.claude.com/docs/en/remote-control">Remote Control</a>: run <code>claude remote-control</code>, then approve permissions and send new prompts from the Claude app. Remote Control needs the Mac awake and online — turn on Auto Mode (and Closed-Lid if the lid is shut) so LidRun keeps that session alive all night.',
